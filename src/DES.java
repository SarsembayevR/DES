...
public class DES {
    ...
    /**
     * Перестановка для генерирования окончательного блока шифртекста
     */
    ...
    //Перестановки связанные с функцией Фейстеля

    /**
     * Функция расширитель Е
     * Результат функции Е(R(i-1)) есть 48-битовая последовательность,
     * которая складывается по модулю 2 (операция xor) с 48-битовым ключом К(i).
     * Получается 48-битовая последовательность, которая разбивается на восемь 6-битовых блоков B(1)B(2)B(3)B(4)B(5)B(6)B(7)B(8).
     */
    ...
    // Перестановки относящиеся к подключу
    ...
    /**
     * Таблица сдвигов для вычисления коюча
     */
    ...
    // методы для выполнения основных перестановок
    ...
    /**
     * Генерация 16 48-битных подключей на основе предоставленного 64-битного ключа
     */
    ...
    /**
     * Шифрование 64-битового блока открытого текста сообщения в 64-разрядный текст
     */
    public static long encryptBlock(long m, /* 64 bits */ long key) {
        long subkeys[] = createSubkeys(key);

        //выполнить начальную перестановку
        long ip = IP(m);

        //разделить 32-битное значение в 16-битную правую и левую половины
        int l = (int) (ip >> 32);
        ...
        //поменять местами два 32-битовых сегмента
        long rl = (r & 0xFFFFFFFFL) << 32 | (l & 0xFFFFFFFFL);
        //применить окончатеьную перестановку
        long fp = FP(rl);
    ...
    /**
     * шифрование входящего сообщения с помощью прилагаемого ключа
     * если сообщение не является кратным 64-бит, то он дополняется нулями
     */
    public static byte[] encrypt(byte[] message, byte[] key) {
        byte[] ciphertext = new byte[message.length];

        //зашифровать каждые 8 байт блока сообщения
    ...
    public static void main(String[] args) {
        ...
        // Эти тесты были получены из пароля запрос-ответ
        // Разговоры наблюдаемые между клиентом и сервером VNC
        test(
